...
directory_path = "D:\\Users\\SMH\\Tim_v2\\validate"
pose_3D_list = []
pose_2D_list = {}
pose_2D_videos = []
pose_2D_joints = []
...
if __name__ == '__main__':
            ...
            for len_video in range(length):
                ...
                if frame is None:
                    ...
                    if len(poses_3d) > 0:
                        ...
                        video_label = video_path.split('\\')[-2]
                        joint_coordinates = poses_2d.tolist()[0]
                        video_data = [video_name, frame_id, video_label]
                        joint_data = joint_coordinates
                        pose_2D_list[video_name].append([frame_id, joint_coordinates, video_label])
                        ...
                        pose_2D_list[video_name].append([frame_id, [], "None"])
                        video_data = [video_name, frame_id, "None"]
                        joint_data = []
                    pose_2D_videos.append(video_data)
                    pose_2D_joints.append(joint_data)
                    frame_id += 1
    ...
    # output_file = directory_path.split('\\')[-1] + "_pose_data.json"
    # with open(os.path.join(directory_path,output_file), "w") as out_file:
    #     json.dump(pose_2D_list, out_file)
    output_file = directory_path.split('\\')[-1] + "_pose_videos.json"
    with open(os.path.join(directory_path,output_file), "w") as out_file:
        json.dump(pose_2D_videos, out_file)
    output_file = directory_path.split('\\')[-1] + "_pose_joints.json"
    with open(os.path.join(directory_path,output_file), "w") as out_file:
        json.dump(pose_2D_joints, out_file)
