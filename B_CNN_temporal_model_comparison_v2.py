...
from keras.layers import Conv2D, MaxPooling2D, Flatten, Dense, Conv1D, Input, concatenate, MaxPooling1D, Dropout, Reshape, LSTM
...
def create_model(input_shape_pose, input_shape_confidence, input_shape_object_classes, input_shape_object_boxes, num_action_labels):
    ...
    # CNN for human pose
    x = Conv2D(64, (3, 3), activation='relu', padding='same')(input_pose)
    ...
    x = Dropout(0.5)(x)
    # Modify LSTM layer
    y = LSTM(units=128, return_sequences=True)(input_confidence)
    y = Flatten()(y)
    # Add dropout for regularization
    y = Dropout(0.5)(y)
    # Modify object class CNN
    z = Conv1D(64, 3, activation='relu', padding='same')(input_object_classes)
    z = MaxPooling1D(2)(z)
    z = Conv1D(64, 3, activation='relu', padding='same')(z)
    z = MaxPooling1D(2)(z)
    z = Flatten()(z)
    # Add dropout for regularization
    z = Dropout(0.5)(z)
    # Modify object boundary box CNN
    w = Conv1D(64, 3, activation='relu', padding='same')(input_object_boxes)
    w = Flatten()(w)
    # Reshape before applying 1D pooling
    w = Reshape((10, 64))(w)
    w = MaxPooling1D(2)(w)
    w = Conv1D(64, 3, activation='relu', padding='same')(w)
    w = MaxPooling1D(2)(w)
    w = Flatten()(w)
    # Add dropout for regularization
    w = Dropout(0.5)(w)
