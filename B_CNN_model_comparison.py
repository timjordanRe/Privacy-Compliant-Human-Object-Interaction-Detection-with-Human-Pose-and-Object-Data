...
from keras.layers import Conv2D, MaxPooling2D, Flatten, Dense, Conv1D, Input, concatenate, MaxPooling1D, Dropout, Reshape
...
n_splits = 20  # You can adjust this as needed
...
def create_model(input_shape_pose, input_shape_confidence, input_shape_object_classes, input_shape_object_boxes, num_action_labels):
    ...
    # 2D CNN for human pose
    x = Conv2D(32, (3,3), activation='relu', padding='same')(input_pose)
    x = MaxPooling2D((2,2))(x)
    x = Flatten()(x)
    # 2D CNN for human pose confidence
    input_confidence = Reshape((10, 19, 1))(input_confidence)
    y = Conv2D(32, (3,3), activation='relu', padding='same')(input_confidence)
    y = MaxPooling2D((2,2))(y)
    y = Flatten()(y)
    # 1D CNN for object classes
    z = Conv1D(32, 3, activation='relu', padding='same')(input_object_classes)
    z = MaxPooling1D(2)(z)
    z = Flatten()(z)
    # 1D CNN for object boundary boxes
    w = Conv1D(32, 3, activation='relu', padding='same')(input_object_boxes)
    w = Flatten()(w)
    # Concatenate all inputs
    merged = concatenate([x, y, z, w])
    # Fully connected layers for classification
    # merged = Dropout(0.5)(merged)  # Add dropout for regularization
