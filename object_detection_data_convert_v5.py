...
# OBJECT VIDEO FORMAT [video_name, frame_id, label, number of recognised items]
object_videos = []
# OBJECT RECOGNITION FORMAT [recognised object, xmin, ymin, xmax, ymax]
object_recognition = []
vid_formats = ('mp4', 'avi', 'mkv')
corrupted_files = []
...
def build_argparser():
    ...
    args.add_argument('-m', '--model', required=False,
                        default="C:\\Users\\mwil0091\\open_model_zoo\\demos\\human_pose_estimation_3d_demo\\python\\public\\yolo-v4-tiny-tf\\FP32\\yolo-v4-tiny-tf.xml",
    ...
    args.add_argument('-i', '--input', required=False,
                      default='/home/smh/Videos/object_detection_dataset/videos_train/looking_at_phone/-7wFG7pYY5Y_000000_000010.mp4',
                      help='Required. An input to process. The input must be a single image, '
                           'a folder of images, video file or camera id.')
    args.add_argument('-ip', '--inputpath', required=False,
                      default="D:\\Users\\SMH\\Tim_v2\\validate",
                      help='Input path to process and store object model outputs')
    ...
    common_model_args.add_argument('--labels', help='Optional. Labels mapping file.', 
                            default="C:\\Users\\mwil0091\\open_model_zoo\\demos\\human_pose_estimation_3d_demo\\python\\labels\\coco_80cl.txt", type=str)
...
def draw_detections(frame, detections, palette, labels, output_transform, frame_id, videopath):
    global object_videos
    ...
    for detection in detections:
        ...
        # don't store person values
        if det_label != "person":
            object_data.append([det_label, (xmin, ymin, xmax, ymax)])
            object_recognition.append([det_label, xmin, ymin, xmax, ymax])
    ...
    print(video_name)
    print()
    label = videopath.split('\\')[-2]
    if len(object_data) > 0:
        frame_data = [frame_id, label, object_data]
    else:
        frame_data = [frame_id, "None", object_data]
    print(frame_data)
    # try:
    #     object_videos[video_name]
    # except Exception as e:
    #     object_videos[video_name] = []
    if len(object_data) > 0:
        object_videos.append([video_name, frame_id, label, len(object_data)])
    else:
        object_videos.append([video_name, frame_id, "None", len(object_data)])
    return frame
...
action_labels = ['bouncing ball (not juggling)',
'brushing teeth',
...
def main():
    ...
    for path, subdirs, files in os.walk(args.inputpath):
        cap = None
        for filename in files:
            if not path.split('\\')[-1] in action_labels:
                continue
            if not filename.split('.')[-1] in vid_formats:
                continue
            frame_id = 0
            vid_input = os.path.join(path, filename)
            try:
                cap = open_images_capture(vid_input, args.loop)
            except SystemExit as e:
                corrupted_files.append(vid_input)
                continue
            next_frame_id = 0
            ...
            while True:
                ...
                if results:
                    ...
                    frame = draw_detections(frame, objects, palette, model.labels, output_transform, frame_id=frame_id, videopath=vid_input)
                    frame_id += 1
    ...
    print(f"TOTAL VIDEOS PROCESSED: {total_vids_processed}")
    output_video_file = args.inputpath.split('\\')[-1] + "_object_videos_yolo.json"
    output_recognition_file = args.inputpath.split('\\')[-1] + "_object_recognition_yolo.json"

    with open(os.path.join(args.inputpath, output_video_file), "w") as outfile:
        json.dump(object_videos, outfile)

    with open(os.path.join(args.inputpath, output_recognition_file), "w") as outfile:
        json.dump(object_recognition, outfile)
    
    with open(os.path.join(args.inputpath, "corrupted_files.txt"), "w") as file:
